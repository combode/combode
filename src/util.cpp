...
const char * const BITCOIN_CONF_FILENAME = "combode.conf";
const char * const BITCOIN_PID_FILENAME = "comboded.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "combode" is a composite category enabling all Combode-related debug output
            if(ptrCategory->count(string("combode"))) {
        ...
        // icbet debugging everything and not debugging specific category, LogPrint does nothing.
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CombodeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CombodeCore
    // Mac: ~/Library/Application Support/CombodeCore
    // Unix: ~/.combodecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CombodeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CombodeCore";
#else
    // Unix
    return pathRet / ".combodecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    if (!boost::filesystem::exists(GetConfigFile())){
        // Create empty combode.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override combode.conf
