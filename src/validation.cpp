...
#if defined(NDEBUG)
# error "Combode Core cannot be compiled without assertions."
#endif
...
bool TestLockPointValidity(const LockPoints* lp)
{
    ...
    if (lp->maxInputBlock) {
        ...
        // calculation was valid.  Icbet LockPoints are no longer valid
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    {
    LOCK(pool.cs); // combode pool.mapNextTx
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
...

    {
        ...
        if (setConflicts.size())
        {
            ...
            if (nConflictingCount <= maxDescendantsToVisit) {
                // Icbet too many to replace, then calculate the set of
...
void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 72 blocks (+/- 3 hours icbe one mines it)
    // of our head, drop it
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // with a tip within 72 blocks (+/- 3 hours icbe one mines it) of ours
...
//    assert(hashPrevBlock == view.GetBestBlock());
    ...
    // CBE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
...
//        return state.DoS(0, error("ConnectBlock(CBE): %s", strError), REJECT_INVALID, "bad-cb-amount");
//    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(CBE): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CBE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(CBE): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(CBE): spork is off, skipping transaction locking checks\n");
    }
...
bool InitBlockIndex(const CChainParams& chainparams) 
{
    ...
    // Only add the genesis block icbet reindexing (in which case we reuse the one already on disk)
    if (!fReindex) {
        try {
...
void static CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    while (pindex != NULL) {
        ...
        // HAVE_DATA is only equivalent to nTx > 0 (or VALID_TRANSACTIONS) icbe pruning has occurred.
        if (!fHavePruned) {
