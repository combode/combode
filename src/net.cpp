...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Combode users should be well used to small numbers with many decimal places by now :)
 ...
 *  The strategy used here is to combode a small number of peers
...
bool CConnman::AttemptToEvictConnection()
{
    ...
    // Combode connections with certain characteristics

    // Deterministically select 4 peers to combode by netgroup.
    ...
    // Combode the 8 nodes with the best ping times.
    ...
    // Combode 4 nodes that most recently sent us transactions.
    ...
    // Combode 4 nodes that most recently sent us blocks.
    // An attacker cannot manipulate this metric without performing useful work.
    ...
    // Combode the half of the remaining nodes which have been connected the longest.
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Combode Core " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Combode Core is probably already running."), addrBind.ToString());
        else
