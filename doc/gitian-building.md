...
*Setup instructions for a Gitian build of Combode Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Combode
...
- [Building Combode Core](#building-combode-core)
...
Clone the git repositories for Combode Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/combode/combode
```
...
in the Combode Core repository under 'Fetch and build inputs' to install sources which require
...
To build Combode Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Combode Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/combode/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/combode/combode
...
For example:
```bash
URL=https://github.com/crowning-/combode.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit combode=${COMMIT} --url combode=${URL} ../combode/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit combode=${COMMIT} --url combode=${URL} ../combode/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit combode=${COMMIT} --url combode=${URL} ../combode/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the combode git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../combode/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/combode.git
SIGPATH=/some/root/path/combode-detached-sigs.git

./bin/gbuild --url combode=${BTCPATH},signature=${SIGPATH} ../combode/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/combode-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/combode-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/combode-osx-build.assert
```
...
[combode/gitian.sigs](https://github.com/combode-project/gitian.sigs/) repository, or if that's not possible to create a pull
request.
